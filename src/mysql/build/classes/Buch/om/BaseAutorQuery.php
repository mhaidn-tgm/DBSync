 ...
 *
 * @method AutorQuery leftJoinBuch($relationAlias = null) Adds a LEFT JOIN clause to the query using the Buch relation
 * @method AutorQuery rightJoinBuch($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Buch relation
 * @method AutorQuery innerJoinBuch($relationAlias = null) Adds a INNER JOIN clause to the query using the Buch relation
 *
...
abstract class BaseAutorQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `name`, `gebdat` FROM `autor` WHERE `id` = :p0';
        try {
    ...
    public function filterByGebDat($gebDat = null, $comparison = null)
    {
        if (is_array($gebDat)) {
            $useMinMax = false;
            if (isset($gebDat['min'])) {
                $this->addUsingAlias(AutorPeer::GEBDAT, $gebDat['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($gebDat['max'])) {
                $this->addUsingAlias(AutorPeer::GEBDAT, $gebDat['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByBuch($buch, $comparison = null)
    {
        if ($buch instanceof Buch) {
            return $this
                ->addUsingAlias(AutorPeer::ID, $buch->getAutor_ID(), $comparison);
